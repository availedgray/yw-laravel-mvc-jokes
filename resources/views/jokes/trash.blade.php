                    ...
                    <table class="mt-4 table bg-white dark:bg-gray-800
                        ...
                        @foreach($jokes as $joke)
                            <tr class="group
                                       ...
                                       dark:text-gray-400">
                                <td class="py-2 pl-2 flex-0 text-left">{{ $joke->title }}</td>
                                <td class="py-2 text-left">{{ $joke->text }}</td>
                                <td class="py-2 text-left">{{ $joke->deleted_at }}</td>
                                <td class="py-2 pr-2 text-right">
                                    <form class="flex flex-row gap-2 items-center justify-end"
                                          action="{{ route('jokes.trash-remove', $joke) }}"
                                          method="POST">
                                        @csrf
                                        @method('delete')
                                        <a href="{{ route('jokes.trash-restore', $joke) }}"
